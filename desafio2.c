...
int main (int argc, char const *argv[])
{
    ...
    // as mesmas que uma unica thread faria e tambem  para  ter  sempre  visivel
    ...
    // argumento opcional e comparar os tempos de execucao  para  uma  thread  e
    // para n = 1 thread(s). O tempo da primeira execucao eh sempre ligeiramente
    // menor devido a nao precisar da estrutura de multithread  (alocar  memoria
    // para a thread, criar uma thread separada da main para  calcular,  usar  o
    // mutex).

    printf(CYAN "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~[ Inicio ]\n" END);

    // Uma execucao fazendo contas com variavel do tipo long,
    // outra fazendo contas com variavel do tipo long a chamadas a rand(),
    // e outra fazendo contas com variavel do tipo long double
    for (int type = 0; type < 3; type++) {
        ...
        if (type == 0) {
            ...
            printf("\tTempo total de execucao: ");
            printf(CYAN "%lu nanossegundos\n" END,
        ...
        // Inicializando rand novamente com uma semente definida  para  garantir
        // que cada execucao fara as mesmas operacoes
        srand((unsigned) 1);
        ...
        if (type == 0) {
            printf("\tTotal              : %lu\n",
                    WORKER_get_long_pseudo_sum());
        } else if (type == 1) {
            printf("\tTotal              : %lu\n",
                    WORKER_get_long_pseudo_sum());
        } else {
            printf("\tTotal              : %.2Lf\n",
                    WORKER_get_long_double_pseudo_sum());
        }
        printf("\tTempo total de execucao: ");
        printf(CYAN "%lu nanossegundos\n" END,
                finish_threads - beginning_threads);
